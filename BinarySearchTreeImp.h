...
template<class Key,class Info>
void BST<Key, Info>::UpdateKey(Key key,Info info) {
    node* NodeToUpDate = FindNode(key,root);
    NodeToUpDate->info = info;
}
...
template<class Key,class Info>
typename BST<Key,Info>::node* BST<Key,Info>::FindNode(Key key,node* Tree){
    if(!Tree)
        throw KeyNotFound();
    if(key > Tree->key)
